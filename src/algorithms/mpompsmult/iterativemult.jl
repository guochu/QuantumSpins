...
@with_kw struct OneSiteIterativeMult <: AbstractMPOMPSMult
	D::Int = 100
	maxiter::Int = 5
	verbosity::Int = 1
	tol::Float64 = 1.0e-8
...
@with_kw struct TwoSiteIterativeMult <: AbstractMPOMPSMult
	D::Int = 100
	maxiter::Int = 5
	verbosity::Int = 1
	tol::Float64 = 1.0e-8
...
changeD(x::OneSiteIterativeMult; D::Int) = OneSiteIterativeMult(D=D, maxiter=x.maxiter, verbosity=x.verbosity, tol=x.tol)
...
IterativeMult(;single_site::Bool=true, kwargs...) = single_site ? OneSiteIterativeMult(; kwargs...) : TwoSiteIterativeMult(; kwargs...)

sweep!(m::MPOMPSIterativeMultCache, alg::AbstractMPOMPSMult, workspace = scalar_type(m)[]) = iterative_error_2(
    vcat(_leftsweep!(m, alg, workspace), _rightsweep!(m, alg, workspace)))

function compute!(m::MPOMPSIterativeMultCache, alg::AbstractMPOMPSMult, workspace = scalar_type(m)[])
	kvals = Float64[]
	iter = 0
	tol = 1.
...
function iterative_mult(mpo::MPO, mps::MPS, alg::AbstractMPOMPSMult = OneSiteIterativeMult())
...
function _leftsweep!(m::MPOMPSIterativeMultCache, alg::OneSiteIterativeMult, workspace = scalar_type(m)[])
    mpsA = m.imps
    mpo = m.mpo
    mpsB = m.omps
    Cstorage = m.hstorage
...
function _rightsweep!(m::MPOMPSIterativeMultCache, alg::OneSiteIterativeMult, workspace = scalar_type(m)[])
    mpsA = m.imps
    mpo = m.mpo
    mpsB = m.omps
    Cstorage = m.hstorage
